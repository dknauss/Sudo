...
**Browser requests (admin UI):** The user sees an interstitial challenge page naming the action and, where the request carries a recognised target parameter, its concrete target. After entering their password (and 2FA code if configured) they are returned to where they were (handler URLs such as `options.php` land on the dashboard instead — a bare GET of them renders nothing usable) and perform the action again under the now-active sudo session. **The intercepted request is never resumed automatically** — reauthenticating proves a person is present, not that they intended the action that was waiting, so the action is always re-issued deliberately. **AJAX and REST requests** receive a `sudo_required` error; an admin notice on the next page load links to the challenge page.
...
For browser-admin requests, Sudo intercepts the request before WordPress processes the sensitive operation. In many common flows this happens on `admin_init`; other actions use surface-specific hooks. The original request is stashed and the user is redirected to a challenge page that names the action and its concrete target. After successful reauthentication the stash is discarded and the user is returned to the screen the request came from (handler URLs such as `options.php` land on the dashboard instead, since a bare GET of them renders nothing usable), holding the sudo session they just earned, to issue the action again themselves. The intercepted request is never re-sent by the plugin. Secrets are never stashed, and such requests are never resumed automatically.
...
* **Known limitation — reauthentication in the browser admin is still worse than before 4.9.0.** Of the four problems reported in 4.9.1, three are fixed above: the misleading notice (#469), the "review the form" instruction given to actions that have no form (#463), and the challenge on a settings save that changed nothing (#445). What remains is the largest one — a gated form still returns you to the correct screen with **every field blank**, so the whole entry is retyped rather than just the secrets (#436 face 1). Two further improvements are tracked on the same issue and not attempted here: naming every rule a form's changes matched rather than only the first (#436 face 3), and gating **before** the form is offered, which would dissolve the problem rather than improve recovery (#436 face 4) and is the most valuable of the three.
...
* **Known limitation — reauthentication in the browser admin is materially worse than before 4.9.0, and is being worked on.** Removing automatic replay closed a real vulnerability, and the convenience cost is larger than the 4.9.0 notes conveyed. Four faces of it, all tracked and none fixed in this release: a gated form returns you to the correct screen with every field blank, so the whole entry is retyped, not just the secrets (#436); the notice says "password and secret fields were not replayed — re-enter them", which implies only the secrets were lost when in fact everything was (#436, #469); a gated GET action is told to "review the form" when there is no form (#463); and `options.critical` gates on the presence of a critical field rather than on a change to it, so every Settings -> General save raises a challenge even when nothing changed (#445). Combined, a routine settings save can mean challenge, reauthenticate, lose the work, and read a notice that misdescribes what happened. Fixing this is a priority for the next release.
...
* **Security — an intercepted action is never resumed automatically** — the gate used to replay your intercepted request once you passed the challenge. Because the stash was keyed to your user account alone, an attacker with a stolen session cookie could plant one, send you to the challenge page, and have your reauthentication perform *their* action. Reauthenticating proved you were present; it never proved you meant to do *that*. Automatic replay is now removed outright — every method, every action, every surface. You are put back where you were: a link-style action returns you to the screen it started from, and a form returns you to the form — but the form comes back EMPTY. Nothing you typed is retained, so a link-style action is a re-click while a form is a full retype. That cost is the point of the fix and not a side effect: an attacker who plants a stashed action gains nothing from your reauthentication, because nothing you did not just do yourself is ever carried out. (Some admin URLs are submission handlers rather than screens, and show nothing useful on their own; a settings save is one. Those land on the dashboard.) You keep the sudo session you just earned, so the retry passes straight through, and a notice explains what happened.
* **Security — a poisoned object cache can no longer forge a sudo session** — the proof was a plain value read through the object cache, so anything able to write into that cache could plant one and obtain a sudo session with no challenge at all. It is now a signed, self-authenticating record read straight from the database, bound to the WordPress login session that created it. Scoped deliberately: this closes the cache-poisoning route. An attacker who can read your `AUTH_SALT` — which is possible when the salts live in the database rather than `wp-config.php` — can still sign a proof, and would additionally need your WordPress login cookie to use it. A side benefit: your **concurrent browsers now hold independent sudo sessions**, so reauthenticating in one no longer silently ends another.
* **Security — a reauth lockout can now be cleared out of band** — five failed reauthentication attempts lock sudo for five minutes, which an attacker could trigger deliberately to stop an administrator using the very tools needed to respond. `wp sudo unlock --user=<id|login>` clears it from the command line, and fires a `wp_sudo_lockout_cleared` action so the clear is auditable.
* **New — scoped break-glass recovery** — `WP_SUDO_RECOVERY_MODE` now accepts a user ID or login, narrowing break-glass access to one named person instead of every administrator. It narrows rather than grants — the named user still needs the administrator capability the action itself requires; recovery mode only waives the sudo challenge. A typo that matches nobody grants nobody. A Site Health critical status flags whenever recovery mode is active and names its scope, so a forgotten constant is caught on the health dashboard.
* **New — see your sudo state while editing** — the block editor hides the admin bar, so the sudo countdown was invisible while writing. A padlock in the editor header now shows whether a session is active, with a warning in the final minute. On WordPress 6.6 and later there is also a live countdown in a "Sudo" editor sidebar; on 6.4 and 6.5 the sidebar API is not available, so those versions get the header padlock and the grant notification only. Informational only: it never starts, extends, or refreshes a session.
* **New — role and capability drift raises an alert** — the optional critical-event alert bridge now pushes on `wp_sudo_role_drift_detected`, so an unauthorised administrator grant reaches email, Slack, or a webhook rather than only Site Health.
* **For developers** — automatic replay after reauthentication is removed outright, for every method, every rule and every admin surface that stashes. A custom rule registered through `wp_sudo_gated_actions` still gates and reauthenticates exactly as before; what changes is that the user always re-issues the action afterwards. Nothing restores replay — no choice of target parameter or `post_fields` allowlist brings it back — so a rule that auto-replayed at 4.8.0 will not at 4.9.0. The `wp_sudo_action_replayed` hook is retained but **dormant**: it no longer fires, because there is no replay to announce. Read `wp_sudo_replay_refused` and its `reason` instead. See "How does sudo gating work?" in the FAQ for the canonical description.

= 4.8.0 - 2026-07-23 =
This release hardens the reauthentication gate against a hijacked admin session by covering three REST write paths that slipped past it, and adds an opt-in audit for privileged-state tampering the gate cannot see.
* **Security — account email changes are now gated** — a new `user.change_email` rule gates email changes on the profile screens and the REST users routes (`/wp/v2/users/{id|me}`) for `POST`, `PUT`, and `PATCH`, firing only on a real change; profile account-mutation saves are now non-replayable. Closes the email-change pivot toward a password-reset takeover of a hijacked session.
* **Security — REST `POST` to `/wp/v2/users` is now gated** — the password-change and role-promotion REST rules matched only `PUT`/`PATCH`, but core registers the users route under `WP_REST_Server::EDITABLE` (`POST, PUT, PATCH`); a cookie-authenticated `POST` could change a password or role with no challenge. Both now gate `POST`, including `/me`.
* **Security — REST writes to critical settings are now gated** — the `options.critical` rule matched raw option names (`siteurl`, `admin_email`), but `/wp/v2/settings` is keyed by each setting's `show_in_rest` name (`url`, `email`), so it never matched; a `POST /wp/v2/settings {"url":"…"}` could repoint the site URL ungated — loading attacker-origin scripts same-origin in wp-admin (an XSS-as-RCE primitive). The rule now also matches the `show_in_rest` aliases.
* **New — role/capability lockdown audit (opt-in, audit-only)** — an optional integrity check that detects privileged-state drift from a file-backed trusted manifest: unauthorized administrators, super admins, or governance-cap holders, and any change to a role's capability definition — including direct database writes the escalation guard cannot see. Inert unless `WP_SUDO_ROLE_MANIFEST` is defined; build a manifest with `wp sudo manifest generate --manifest-path=<file>`, then check drift via `wp sudo manifest diff`, Site Health, or the scheduled sweep. Reads users and role definitions fresh from the database, so a raw grant is not hidden by a persistent object cache. Multisite audits the current blog (network-wide sweep planned).
* **Readable governance capabilities on the user profile (UX)** — WP Sudo's governance capabilities no longer render as a run-on list of raw slugs under core's "Additional Capabilities" heading; a dedicated "Sudo capabilities" block lists each granted capability by its human-readable label, and another plugin's capabilities are never hidden.
* **Maintenance** — hardened E2E CI startup against hangs; updated `adm-zip` for CVE-2026-39244; routine dependency and GitHub Actions bumps. Published a spec and proof-of-concept patches proposing the action-gate/recent-auth concept for WordPress core.
