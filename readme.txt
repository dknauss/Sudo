...
**Browser requests (admin UI):** The user sees an interstitial challenge page naming the action and, where the request carries a recognised target parameter, its concrete target. After entering their password (and 2FA code if configured) they are returned to where they were (handler URLs such as `options.php` land on the dashboard instead, and `user-edit.php` on the Users list — a bare GET of them renders nothing usable) and perform the action again under the now-active sudo session. **The intercepted request is never resumed automatically** — reauthenticating proves a person is present, not that they intended the action that was waiting, so the action is always re-issued deliberately. **AJAX and REST requests** receive a `sudo_required` error; an admin notice on the next page load links to the challenge page.
