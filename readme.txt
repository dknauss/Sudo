...
* **Security — a stashed action is no longer resumed without proof it was yours** — the gate used to replay your intercepted request automatically once you passed the challenge. Because the stash was keyed to your user account alone, an attacker with a stolen session cookie could plant one, send you to the challenge page, and have your reauthentication perform *their* action. Reauthenticating proved you were present; it never proved you meant to do *that*. Now the challenge **names the action and its target** before you enter your password, and the action only resumes in the same browser that started it. If either check fails nothing is replayed — but you are put back where you were: a link-style action returns you to the screen it started from, and a form returns you to the form, so re-doing it is a re-click or a re-submit rather than a hunt. (Some admin URLs are submission handlers rather than screens, and show nothing useful on their own; a settings save is one. Those still land on the dashboard.) You keep the sudo session you just earned, so the retry passes straight through, and a notice explains what happened. Requires HTTPS; on plain HTTP the action is never auto-resumed.
