...
* **Security — a stashed action is no longer resumed without proof it was yours** — the gate used to replay your intercepted request automatically once you passed the challenge. Because the stash was keyed to your user account alone, an attacker with a stolen session cookie could plant one, send you to the challenge page, and have your reauthentication perform *their* action. Reauthenticating proved you were present; it never proved you meant to do *that*. Now the challenge **names the action and its target** before you enter your password, and the action only resumes in the same browser that started it. If either check fails nothing is replayed. A link-style action returns you to the screen it started from; a form submission lands on the dashboard, because its re-submit needs the form filled in again either way. Both carry a notice, and both leave you holding the sudo session you just earned so the retry passes straight through. Requires HTTPS; on plain HTTP the action is never auto-resumed.
