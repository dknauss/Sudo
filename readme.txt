...
**Browser requests (admin UI):** The user sees an interstitial challenge page naming the action and, where the request carries a recognised target parameter, its concrete target. After entering their password (and 2FA code if configured) they are returned to where they were and perform the action again under the now-active sudo session. **The intercepted request is never resumed automatically** — reauthenticating proves a person is present, not that they intended the action that was waiting, so the action is always re-issued deliberately. **AJAX and REST requests** receive a `sudo_required` error; an admin notice on the next page load links to the challenge page.
...
* **For developers** — automatic replay after reauthentication now requires the challenge to have named the whole effect. A custom rule registered through `wp_sudo_gated_actions` whose fields are not recognised targets still gates and reauthenticates exactly as before, but is generally not resumed automatically — the user re-issues it. One case is not yet reliably refused: a link-style custom action whose effect rides an unrecognised parameter while a recognised one is also present. That is tracked and is not closed in 4.9.0. See "Replay eligibility requires the confirmation to name the whole effect" in the developer reference. The `wp_sudo_action_replayed` hook is unchanged and still fires, now only for same-browser resumed actions.
