...
* **Security — a stashed action is no longer resumed without proof it was yours** — the gate used to replay your intercepted request automatically once you passed the challenge. Because the stash was keyed to your user account alone, an attacker with a stolen session cookie could plant one, send you to the challenge page, and have your reauthentication perform *their* action. Reauthenticating proved you were present; it never proved you meant to do *that*. Now the challenge **names the action and its target** before you enter your password, and the action only resumes in the same browser that started it. If either check fails nothing is replayed — you land back on the page you started from, holding the sudo session you just earned, and re-do the action yourself. Requires HTTPS; on plain HTTP the action is never auto-resumed.
* **Security — the sudo session proof can no longer be forged** — the proof was a plain value read through the object cache, so anything able to write into that cache could plant one and obtain a sudo session with no challenge at all. It is now a signed, self-authenticating record read straight from the database, bound to the WordPress login session that created it. A side benefit: your **concurrent browsers now hold independent sudo sessions**, so reauthenticating in one no longer silently ends another.
...
* **New — scoped break-glass recovery** — `WP_SUDO_RECOVERY_MODE` now accepts a user ID or login, granting break-glass access to one named person instead of every administrator. A typo that matches nobody grants nobody. A Site Health critical status flags whenever recovery mode is active and names its scope, so a forgotten constant is caught on the health dashboard.
* **New — see your sudo state while editing** — the block editor hides the admin bar, so the sudo countdown was invisible while writing. A padlock in the editor header now shows whether a session is active, with a live countdown in the editor sidebar and a warning in the final minute. Informational only: it never starts, extends, or refreshes a session.
...
* **For developers** — automatic replay after reauthentication now requires the challenge to have named the whole effect. A custom rule registered through `wp_sudo_gated_actions` whose fields are not recognised targets still gates and reauthenticates exactly as before, but the user re-issues the action instead of it resuming by itself. See "Replay eligibility" in the developer reference. The `wp_sudo_action_replayed` hook is unchanged and still fires, now only for same-browser resumed actions.
