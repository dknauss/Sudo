...
test.describe("Visual regression baselines", () => {
   ...
   */
  test("VISN-01: challenge page card element baseline", async ({ page }) => {
    ...
    await page.waitForFunction(
      () =>
        typeof (window as Window & { wpSudoChallenge?: unknown })
          .wpSudoChallenge !== "undefined",
    );
  ...
  test("VISN-02: settings page form element baseline", async ({ page }) => {
    ...
    await expect(page.locator(".wrap")).toHaveScreenshot("settings-form.png", {
      threshold: 0.05,
      mask: [muStatus],
    });
  });
  ...
  test("VISN-03: admin bar node in active session state baseline", async ({
    ...
    await expect(page).toHaveScreenshot("admin-bar-active.png", {
  ...
  test("VISN-04: admin bar node in expiring state baseline", async ({
    ...
    await expect(page).toHaveScreenshot("admin-bar-expiring.png", {
      clip: { x: 0, y: 0, width: 1280, height: 32 },
      mask: [timerNode.locator(".ab-label")],
      threshold: 0.1,
      maxDiffPixels: 200,
