...
status: v4.8.0 released 2026-07-23. Post-release `main` is documentation + CI tooling only (nothing version-bumped). This file is a point-in-time pointer, not a second source of truth — use the canonical sources below.
---
...
- **Since v4.8.0 on `main` — documentation + CI tooling only:** the **ROADMAP
  restructure** (forward-only Now/Next/Later; design essays promoted to standalone
  docs; the feature backlog migrated to GitHub issues #238–#257) and the
  **persistent-options metrics gate** (`bin/scan-persistent-options.php` +
