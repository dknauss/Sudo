...
Open threads: Phase 14 (ORG-01…07) on branch docs/wordpress-org-readiness — **14-02 COMPLETE** (ORG-03 brand strings in includes/class-site-health|gate|admin.php + developer-reference divergence section; commit c10ce12, 2026-06-20) and **14-03 COMPLETE** (ORG-04/06/07: SECURITY.md, redaction-status reconcile, wporg-submission-checklist.md). **14-01 NOT started** (readme.txt + wp-sudo.php header, reviewer-gated) — SCOPE CHANGED: per 14-02-SUMMARY the plugin header **stays "WP Sudo"** and the plan's older "Sudo Security" reconciliation is stale, so 14-01 must drop that rename. **14-04 (screenshots)** now SCRIPTABLE via `npm run screenshots` (needs wp-env + a visual eyeball, no longer a hard browser block). Phase 15 (ENV-01…03) **substantially delivered by external PR #98**. Tag v4.0.0 after 14-01/14-04 close + Phase 15 verified.
Resume file: None
