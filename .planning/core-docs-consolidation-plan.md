...
- ⚠️ **SUPERSEDED (see #360):** the decision below was reversed. The PoC is now at `docs/archive/core-sudo-gate-poc-patches.md`, replaced by the runnable slice at `poc/install-package-gate/`. It was kept separate on the reasoning that it was the most persuasive engineering artifact; in practice it drifted alongside the spec and accumulated nine defects, none of which anything could catch because pseudocode has no CI.
