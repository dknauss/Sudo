...
test.describe( 'Challenge flow', () => {
    ...
    test( 'CHAL-02: challenge page renders required form elements', async ( {
        ...
        // Cancel link must be present (standard anchor, not a button).
        // Source: class-challenge.php render_page() — <a class="button">Cancel</a> (verified)
        await expect(
            page.locator( '.wp-sudo-challenge-cancel' ).or( page.locator( 'a.button:has-text("Cancel")' ) ),
...
test.describe( 'MU-plugin AJAX install/uninstall', () => {
    ...
    test.describe( 'Install and uninstall flow', () => {
        ...
        test( 'MUPG-01: install button triggers AJAX and shows Installed status after reload', async ( {
            ...
            // Note: spinner visibility is transient (1000ms before reload) — just attempt to observe.
            // Don't make this a hard assertion since timing can vary.
        ...
        test( 'MUPG-02: uninstall button removes MU-plugin and shows Not installed after reload', async ( {
            page,
        } ) => {
            // This test runs after MUPG-01 (MU-plugin is now installed).
    ...
    /**
     * MUPG-03: MU-plugin install fails with 403 when no sudo session is active.
     *
     ...
     */
    test( 'MUPG-03: install AJAX returns error when no sudo session is active', async ( {
        page,
        context,
    } ) => {
...
The wp-sudo-gate.php file should not be present after the test run (afterAll cleans up).
  ...
  <done>
- MUPG-01: Install button visible → click → page reloads → status shows "Installed"
- MUPG-02: Uninstall button visible → click → page reloads → status shows "Not installed"
- MUPG-03: Install AJAX returns HTTP 403 with code=sudo_required when no session active
...
<verification>
- `npx playwright test --config tests/e2e/playwright.config.ts tests/e2e/specs/challenge.spec.ts tests/e2e/specs/mu-plugin.spec.ts` passes all 6 tests
...
- MUPG-01: Install button triggers AJAX; status shows Installed after page reload
- MUPG-02: Uninstall button removes plugin; status shows Not installed after reload
- MUPG-03: Install AJAX returns 403 + sudo_required when no session is active
</success_criteria>
