...
- **BREAKING — the `wp_sudo_action_replayed` hook is removed (5.0.0):** a direct consequence of the `#322` fix below. The gate no longer replays a stashed request, so the hook had no reachable call site and was removed rather than left as a hook that can never fire. It was a documented public extension point (`docs/developer-reference.md`, event code `1900009`), and per `VERSIONING.md` removing a documented hook is a **MAJOR** change — this is what makes the release `5.0.0` rather than a minor. **Nothing replaces it:** there is no replay event left to observe, so a listener that counted replays should read its silence as "replay no longer occurs", not as a gap. What deliberately remains: `Event_Recorder` still maps the event name and the dashboard widget still offers the "Replayed" filter and label, so `action_replayed` rows written *before* the upgrade stay readable — event code `1900009` therefore still resolves for historical rows and must never be reassigned. Bridges that subscribe to the hook keep their (now inert) `add_action` registrations, so no bridge needs a version check to load. The settings-page "Audit hooks" help list no longer advertises it.
- **Security — a stashed action is never auto-replayed after reauthentication (fix):** the gate stashed an intercepted admin request and replayed it once the user completed the sudo challenge. That made the challenge a **confused deputy**: an attacker who could plant a stash entry (and then get the victim to reauthenticate for any reason) had the victim's own reauth execute the *attacker's* transaction, with the victim's capabilities and a fully valid sudo session. The reauth proved the user was present; it never proved they intended *that* action, because the action was chosen before the challenge began and never shown to them. The gate now **fails closed**: reauthentication grants the sudo window and nothing more — no stashed request is replayed, and the user re-issues the action themselves against a session they now hold. This is a deliberate UX cost (a gated form must be resubmitted) accepted in exchange for removing the binding gap between "who authenticated" and "what executes". Guarded by `tests/Unit/ChallengeTest.php` (written failing first) and `tests/Unit/ActionRegistryTest.php`. (#322)
