...
- **In-editor reauthentication for the block editor (new):** when a block-editor REST request is soft-blocked with `sudo_required`, WP Sudo now surfaces an in-editor snackbar with a "Reauthenticate" action that opens the challenge page (instead of the editor dead-ending on an opaque 403). Editor state is preserved; the user grants a sudo session and retries. An `apiFetch` middleware detects the `sudo_required` code — including one nested in a `/batch/v1` envelope — and consumes the server-emitted `challenge_url` verbatim (never rebuilt in JS), degrading to a plain message if that URL is absent or not a same-origin http(s) URL. Supporting server plumbing localizes the grant config (`wpSudoEditorReauth`, always available so a long-open editor can reauthenticate even mid-session) and adds a logged-in-only AJAX endpoint (`wp_sudo_refresh_grant_nonce`) that re-mints a fresh grant nonce for an editor left open past the ~24 h nonce lifetime; it grants nothing. This is the link-out increment — the in-editor password/2FA modal and automatic request re-dispatch are deferred to a later release.
