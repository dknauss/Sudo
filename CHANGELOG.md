...
- **Escalation-guard authority hardening (security):** the opt-in admin-escalation guard now requires the acting user to hold the promoting authority — `promote_users` for administrator grants, and existing super-admin status for `grant_super_admin` — *in addition to* an active/in-grace sudo session, before allowing the grant. Sudo is reauthentication, not authorization: a low-privilege account (subscriber/customer) can hold a sudo session, so requiring the session alone let the backstop wave through an escalation that reached a broken-access-control route. Blocked grants still fire `wp_sudo_escalation_blocked`.
