...
- **Security — a stashed action is no longer resumed without proof it was yours (fix):** the gate stashed an intercepted admin request and replayed it once the challenge was passed. That made reauthentication a **confused deputy**: the stash was keyed to `user_id` alone, so an attacker able to plant one — a cloned session from a stolen cookie, no password needed — could lure the victim to the challenge and have the victim's own reauthentication carry out the *attacker's* transaction, with the victim's capabilities and a fully valid sudo session. The reauth proved a person was present; it never proved they intended *that* action, because the action was chosen before the challenge began and was never shown to them. GET actions were worst: replaying one was a plain redirect to the destructive URL. Replay is now authorised by **two** controls. First, a per-browser binding proof: `Request_Stash::mint_binding_proof()` issues a `__Host-wp_sudo_stash_proof` cookie, minted **only** when the gated request carried `Sec-Fetch-Site: same-origin`, so a stash planted from another browser cannot be resumed in the victim's. Second, **informed confirmation** — the challenge page now names the concrete target of the action ("Target: `evil/evil.php`") from `Request_Stash::TARGET_PARAMS`, so the password submit is consent to a *known* action rather than a blank cheque; this is the control that still holds if the binding is bypassed. When either control is unsatisfied the gate **fails closed**: nothing is replayed, and the user is returned to the screen the request came from — with its effect-bearing query stripped and re-validated same-origin — carrying a notice and the sudo session they just earned, to re-issue the action themselves. The landing is always derived from the intercepted request URL, never from the stashed `return_url`: that value comes from `wp_get_referer()` and is therefore chosen by whoever planted the stash, so redirecting to it would re-open the same confused deputy by another route. Where the stripped URL is a form *handler* rather than a screen, the neutral dashboard is used instead, since landing on the handler would cost the user the explanatory notice as well as the form: `options.php` renders the raw All Settings dump on GET; `admin-post.php`, `admin.php` and `update.php` render nothing without their action/page parameter; `user-edit.php` calls `wp_die( 'Invalid user ID.' )`; `admin-ajax.php` never renders a page; and network `edit.php` redirects to the network dashboard, discarding the query. The handler test reads the URL path rather than `is_network_admin()`, which is false on the admin-ajax path where the landing is computed. Because that list fails open — an unenumerated page would be treated as a good landing — a guard test walks `Action_Registry`'s `pagenow` set in both single-site and multisite form, resolves each to a full URL, and fails until every POST-capable rule's landing has been checked against upstream (#429). **Requires HTTPS** — the binding cookie is issued only over a secure connection, so plain-HTTP installs never engage binding and stay on the fail-closed path permanently. A Site Health test surfaces this rather than leaving it to be discovered. Guarded by `tests/Unit/ChallengeTest.php`, `tests/Unit/RequestStashTest.php` and `tests/e2e/specs/challenge.spec.ts`. (#322)
