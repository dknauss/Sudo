...
| [`blueprint-multisite.json`](blueprint-multisite.json) | Public/reviewer scenario | Multisite/network-admin demo — converts the standard-seed single site to a network (`enableMultisite`), network-activates WP Sudo, and lands the super admin on the **network** Sudo settings page (`/wp-admin/network/settings.php?page=wp-sudo-settings`) | `main` while featured; tag-pinned copy after release if kept as a public demo | readme.md while featured; [`docs/ui-ux-testing-prompts.md`](docs/ui-ux-testing-prompts.md) §6d |
