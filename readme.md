...
- **Front-end page loads:** no *added* database queries for visitors — the always-on hooks read only constants and the autoloaded roles option (a static-hook characterization, not a runtime `$wpdb->num_queries` measurement) — and at most one cached user-meta read for a logged-in user (the admin-bar session check). Database activity is confined to the specific gated action being confirmed, not to normal browsing.
