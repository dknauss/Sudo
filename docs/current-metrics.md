...
|---|---:|---|
| Unit tests | 1,272 tests | `composer test:unit` |
| Unit assertions | 3,803 assertions | `composer test:unit` |
...
| Tests PHP lines (`tests/`) | 44,676 | `find ./tests -type f -name "*.php" -print0 | xargs -0 wc -l | tail -1 | awk '{print $1}'` |
| Production + tests PHP lines | 65,688 | sum of the two rows above |
| Test-to-production ratio | 2.13:1 | `44629 / 21012` |
| Total repo PHP lines (excluding `vendor/`, `vendor_test/`, `.tmp/`, `.git/`, `.claude/`) | 67,963 | `find . -type f -name "*.php" ! -path "*/vendor/*" ! -path "*/vendor_test/*" ! -path "*/.tmp/*" ! -path "*/.git/*" ! -path "*/.claude/*" -print0 | xargs -0 wc -l | tail -1 | awk '{print $1}'` |
