...
|---|---:|---|
| Unit tests | 992 tests | `composer test:unit` |
| Unit assertions | 3,017 assertions | `composer test:unit` |
...
| Tests PHP lines (`tests/`) | 36,086 | `find ./tests -type f -name "*.php" -print0 | xargs -0 wc -l | tail -1 | awk '{print $1}'` |
| Production + tests PHP lines | 53,243 | sum of the two rows above |
| Test-to-production ratio | 2.10:1 | `36086 / 17157` |
| Total repo PHP lines (excluding `vendor/`, `vendor_test/`, `.tmp/`, `.git/`) | 53,515 | `find . -type f -name "*.php" ! -path "*/vendor/*" ! -path "*/vendor_test/*" ! -path "*/.tmp/*" ! -path "*/.git/*" -print0 | xargs -0 wc -l | tail -1 | awk '{print $1}'` |
