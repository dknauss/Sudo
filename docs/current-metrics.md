...
|---|---:|---|
| Unit tests | 1,129 tests | `composer test:unit` |
| Unit assertions | 3,358 assertions | `composer test:unit` |
...
| Tests PHP lines (`tests/`) | 39,819 | `find ./tests -type f -name "*.php" -print0 | xargs -0 wc -l | tail -1 | awk '{print $1}'` |
| Production + tests PHP lines | 58,891 | sum of the two rows above |
| Test-to-production ratio | 2.09:1 | `39819 / 19072` |
| Total repo PHP lines (excluding `vendor/`, `vendor_test/`, `.tmp/`, `.git/`, `.claude/`) | 60,344 | `find . -type f -name "*.php" ! -path "*/vendor/*" ! -path "*/vendor_test/*" ! -path "*/.tmp/*" ! -path "*/.git/*" ! -path "*/.claude/*" -print0 | xargs -0 wc -l | tail -1 | awk '{print $1}'` |
