...
| Tests PHP lines (`tests/`) | 37,273 | `find ./tests -type f -name "*.php" -print0 | xargs -0 wc -l | tail -1 | awk '{print $1}'` |
| Production + tests PHP lines | 54,996 | sum of the two rows above |
| Test-to-production ratio | 2.10:1 | `37273 / 17723` |
| Total repo PHP lines (excluding `vendor/`, `vendor_test/`, `.tmp/`, `.git/`, `.claude/`) | 56,063 | `find . -type f -name "*.php" ! -path "*/vendor/*" ! -path "*/vendor_test/*" ! -path "*/.tmp/*" ! -path "*/.git/*" ! -path "*/.claude/*" -print0 | xargs -0 wc -l | tail -1 | awk '{print $1}'` |
...
| E2E tests | 76 | `npx playwright test --config tests/e2e/playwright.config.ts --list` (verified 2026-07-14; 12 active `editor-reauth.spec.ts` in-editor-modal tests — EDITOR-01 modal-cancel→link-out, EDITOR-02 batch detect-and-surface (Q2), EDITOR-03/05 no-safe-URL degradation (C4), EDITOR-06 modal grant, EDITOR-07 rejected-submission recovery, EDITOR-08 2fa_pending links-out (Step 2), EDITOR-09 stale-nonce refresh recovery (Step 3), EDITOR-10 owner-scoped concurrent re-dispatch + EDITOR-11 shared-modal cancel fallback (Step 4, Q3), EDITOR-12 modal never echoes the rule label (Step 5, Q4) — verified green against a live WP Sudo env, no remaining `test.fixme`) | v4.0.0 |
