...
|---|---:|---|
| Unit tests | 1,151 tests | `composer test:unit` |
| Unit assertions | 3,396 assertions | `composer test:unit` |
...
| Metric | Value | Verification |
|---|---:|---|
| Production PHP lines (`includes/`, `wp-sudo.php`, `uninstall.php`, `mu-plugin/`, `bridges/`) | 19,300 | `find ./includes ./wp-sudo.php ./uninstall.php ./mu-plugin ./bridges -type f -name "*.php" -print0 | xargs -0 wc -l | tail -1 | awk '{print $1}'` |
| Tests PHP lines (`tests/`) | 40,283 | `find ./tests -type f -name "*.php" -print0 | xargs -0 wc -l | tail -1 | awk '{print $1}'` |
| Production + tests PHP lines | 59,583 | sum of the two rows above |
| Test-to-production ratio | 2.09:1 | `40283 / 19300` |
| Total repo PHP lines (excluding `vendor/`, `vendor_test/`, `.tmp/`, `.git/`, `.claude/`) | 61,092 | `find . -type f -name "*.php" ! -path "*/vendor/*" ! -path "*/vendor_test/*" ! -path "*/.tmp/*" ! -path "*/.git/*" ! -path "*/.claude/*" -print0 | xargs -0 wc -l | tail -1 | awk '{print $1}'` |
...
| Gated rules (single-site) | 28 | `grep "'id'" includes/class-action-registry.php \| grep -v network \| grep -v "rule\[" \| wc -l` | v3.2.0 |
...
- `composer test` (1,135 tests) + `composer analyse` + `npm run test:e2e` passed on 2026-07-25 against a live WP 7.0 wp-env. In-editor sudo **session-status indicator** (#262 / #182, client UI): a new build-free module `admin/js/wp-sudo-session-indicator.js`, enqueued as a SEPARATE handle from the reauth modal in `Plugin::enqueue_editor_reauth()` (deps `wp-element`, `wp-data`, `wp-notices`, `wp-i18n`, `wp-plugins`, `wp-editor`, `wp-dom-ready`). Part A: an announce-once `core/notices` success snackbar (WP 6.4+); Part B: a feature-detected `wp.editor.PluginSidebar` (WP 6.6+) whose "Sudo" panel shows a live `M:SS` countdown. The countdown is anchored to an **absolute `Date.now()` deadline** (re-derived on `visibilitychange`/`focus`) so a throttled background tab never shows a confidently-wrong "still active" time (design-review objection 1). Two feeds, no endpoint, no polling: #1 the `is_active()`-gated page-load `remaining` localize (already shipped, #204) and #2 a `wp-sudo-session-granted` window CustomEvent the reauth modal dispatches on a successful `authenticated` grant — the modules are decoupled (neither imports the other). The page-load snackbar is `domReady`-deferred and `sessionStorage`-gated announce-once so reloads stay quiet; a genuine grant always confirms (design-review objections 2–3). Informational only: it never mints, extends, or refreshes a session (+1 unit test; +5 E2E in `editor-session-indicator.spec.ts`; the 17 `editor-reauth.spec.ts` tests stay green after the modal now emits feed #2).
