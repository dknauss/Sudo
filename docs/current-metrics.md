...
|---|---:|---|
| Unit tests | 1,041 tests | `composer test:unit` |
| Unit assertions | 3,131 assertions | `composer test:unit` |
...
| Tests PHP lines (`tests/`) | 37,985 | `find ./tests -type f -name "*.php" -print0 | xargs -0 wc -l | tail -1 | awk '{print $1}'` |
| Production + tests PHP lines | 56,028 | sum of the two rows above |
| Test-to-production ratio | 2.11:1 | `37985 / 18043` |
| Total repo PHP lines (excluding `vendor/`, `vendor_test/`, `.tmp/`, `.git/`, `.claude/`) | 57,169 | `find . -type f -name "*.php" ! -path "*/vendor/*" ! -path "*/vendor_test/*" ! -path "*/.tmp/*" ! -path "*/.git/*" ! -path "*/.claude/*" -print0 | xargs -0 wc -l | tail -1 | awk '{print $1}'` |
