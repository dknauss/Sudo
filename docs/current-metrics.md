...
| E2E tests | 75 | `npx playwright test --config tests/e2e/playwright.config.ts --list` (verified 2026-07-14; 11 active `editor-reauth.spec.ts` in-editor-modal tests — EDITOR-01 modal-cancel→link-out, EDITOR-02 batch detect-and-surface (Q2), EDITOR-03/05 no-safe-URL degradation (C4), EDITOR-06 modal grant, EDITOR-07 rejected-submission recovery, EDITOR-08 2fa_pending links-out (Step 2), EDITOR-09 stale-nonce refresh recovery (Step 3), EDITOR-10 owner-scoped concurrent re-dispatch + EDITOR-11 shared-modal cancel fallback (Step 4, Q3) — verified green against a live WP Sudo env, no remaining `test.fixme`) | v4.0.0 |
