...
|---|---:|---|
| Unit tests | 981 tests | `composer test:unit` |
| Unit assertions | 2,983 assertions | `composer test:unit` |
...
| Tests PHP lines (`tests/`) | 35,743 | `find ./tests -type f -name "*.php" -print0 | xargs -0 wc -l | tail -1 | awk '{print $1}'` |
| Production + tests PHP lines | 52,748 | sum of the two rows above |
| Test-to-production ratio | 2.10:1 | `35743 / 17005` |
| Total repo PHP lines (excluding `vendor/`, `vendor_test/`, `.tmp/`, `.git/`) | 53,020 | `find . -type f -name "*.php" ! -path "*/vendor/*" ! -path "*/vendor_test/*" ! -path "*/.tmp/*" ! -path "*/.git/*" -print0 | xargs -0 wc -l | tail -1 | awk '{print $1}'` |
