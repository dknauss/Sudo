...
|---|---:|---|
| Unit tests | 1,132 tests | `composer test:unit` |
| Unit assertions | 3,361 assertions | `composer test:unit` |
...
| Tests PHP lines (`tests/`) | 39,849 | `find ./tests -type f -name "*.php" -print0 | xargs -0 wc -l | tail -1 | awk '{print $1}'` |
| Production + tests PHP lines | 58,921 | sum of the two rows above |
| Test-to-production ratio | 2.09:1 | `39849 / 19072` |
| Total repo PHP lines (excluding `vendor/`, `vendor_test/`, `.tmp/`, `.git/`, `.claude/`) | 60,410 | `find . -type f -name "*.php" ! -path "*/vendor/*" ! -path "*/vendor_test/*" ! -path "*/.tmp/*" ! -path "*/.git/*" ! -path "*/.claude/*" -print0 | xargs -0 wc -l | tail -1 | awk '{print $1}'` |
