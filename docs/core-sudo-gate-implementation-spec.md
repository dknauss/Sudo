...
- **Multisite: the window is network-wide in v1.** `WP_Session_Tokens` are user-global across a network, so `reauth_at` rides the network-wide session — one reauth satisfies the gate on every site in the network, not only the site where the challenge happened. This matches WP Sudo's network-global session model and is the simplest correct behavior (the actor proved presence once), but it is broader than an operator on a mixed-trust network may expect. Per-site tightening is deferred to the `scope` tag and Phase 2 policy (§8, §11), not v1.
