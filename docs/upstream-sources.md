...
1. Read the file. Not `grep` — read enough of it to name the **enclosing symbol**, the
   ...
   that it belongs to the control you are describing. That exact mistake shipped in #288
   and was corrected in #317.
...
4. In prose, write `see GB-ICON-TOGGLE in docs/upstream-sources.md` rather than another
...
|---|---|---|---|---|---|
| GB-PRESSED-FILL | https://raw.githubusercontent.com/WordPress/gutenberg/trunk/packages/components/src/button/style.scss | 347 | `background: $components-color-foreground;` | `&.is-pressed` block in `.components-button` | Core's only background change on a pinned-item button is a neutral pressed fill — it applies no *semantic* background colour, which is why WP Sudo carries state on the glyph and not a chip. |
...
| GB-PINNED-CLASS | https://raw.githubusercontent.com/WordPress/gutenberg/trunk/packages/interface/src/components/pinned-items/index.js | 23 | `'interface-pinned-items'` | `PinnedItems` | The container class the stylesheet scopes to, so the Options-menu copy of the same toggle is not painted. |
