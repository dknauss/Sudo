...
| GB-CLI-ASSOC | https://raw.githubusercontent.com/wp-cli/wp-cli/main/php/WP_CLI/Configurator.php | 298 | `preg_match( '\|^--([^=]+)=(.*)\|s', $arg, $matches )` | `Configurator::extract_assoc()` | A WP-CLI associative-argument value is never a PHP int. `extract_assoc()` assigns `false` for `--no-x` (L294), `true` for a bare `--x` (L296) or `-x` (L300), and otherwise the regex-captured remainder of `--x=v` — always a string. The decision this supports: `CLI_Command::resolve_user_id()` treats a digits-only string as a user ID rather than requiring an `is_int()` check, because an `is_int()` check would reject every real `--user=123`. |
| GB-AUTH-COOKIE-PASSFRAG | https://raw.githubusercontent.com/WordPress/wordpress-develop/trunk/src/wp-includes/pluggable.php | 863 | `$key = wp_hash( $username . '\|' . $pass_frag . '\|' . $expiration . '\|' . $token, $scheme );` | `wp_validate_auth_cookie()` | The auth-cookie validation key is derived in part from `$pass_frag`, a fragment of the user's stored password hash (set two lines above from `$user->user_pass`). Changing the password changes `$pass_frag`, which invalidates every previously issued auth cookie's HMAC — including one a session-hijacking attacker holds. Supports `CLI_Command::unlock()`'s remediation guidance: `wp sudo revoke` clears only the sudo-layer proof (`Sudo_Session::deactivate()`), not the actual WordPress login session, so it does not stop an attacker who still holds that login cookie — a password reset (`wp user reset-password <id>`) does, via this mechanism. |
