...
| GB-SESSION-TOKEN-COOKIE | https://raw.githubusercontent.com/WordPress/wordpress-develop/trunk/src/wp-includes/user.php | 3672 | `$cookie = wp_parse_auth_cookie( '', 'logged_in' );` | `wp_get_session_token()` | The session token that keys a nonce is read straight out of the `logged_in` auth cookie. Completes GB-NONCE-TOKEN: the stolen cookie *is* the token, so nonce possession follows from cookie theft and cannot distinguish attacker from victim. |
