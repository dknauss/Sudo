...
- **Stabilize the nginx-multisite release-confidence smoke lane** — the `nginx-multisite-smoke` job (MSTACK-01/02/03 in `tests/e2e/specs/multisite-stack-smoke.spec.ts`) has never been green: it fails on Playwright *element-not-stable / actionability* timeouts (`#submit`, `#wp-admin-bar-wp-sudo-active`) on the heavier multisite stack — a test-robustness/rendering issue, not a behavioral or `4.6.0`-related failure (see `docs/release-environment-log.md` → `4.6.0` section). It is currently **de-scoped** from the gate (`continue-on-error` + excluded from the aggregate in `.github/workflows/release-confidence.yml`). Fix: open the failing run's Playwright trace to find what keeps those controls from stabilizing (leading hypothesis: continuous admin-bar/countdown reflow while a session is active), harden the interactions (settle/`scrollIntoViewIfNeeded`/force-stable click) or the page, confirm green, then remove the `continue-on-error` and restore the job to the aggregate loop.
