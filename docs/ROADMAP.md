...
- **Critical-event alerting bridge** — Today WP Sudo *observes but does not alert*: the packaged Stream (`bridges/wp-sudo-stream-bridge.php`) and WSAL (`bridges/wp-sudo-wsal-sensor.php`) bridges *log* audit events, and suspicious states surface passively in Site Health and the dashboard widget, but nothing is pushed to an operator by default. Add a first-party bridge (mu-plugin, same inert-when-unavailable pattern as the existing bridges) that wires the **critical / high-severity** hooks to a real notification channel — `wp_mail` out of the box, with a filter to send Slack/Teams/webhook via `wp_remote_post`. Scope to the criticals: `wp_sudo_capability_tampered`, `wp_sudo_escalation_blocked`, `wp_sudo_lockout`, `wp_sudo_gated_actions_missing_builtin_rules`, and a **throttled** `wp_sudo_recovery_mode_active` (it fires per admin-page load — rate-limit to ~one alert per user per hour). Include per-event enable toggles and a dedupe/throttle window so an incident cannot flood the channel. Until it ships, the DIY snippet in [`developer-reference.md`](developer-reference.md) → "Wiring critical audit hooks to alerts" covers the same hooks. Distinct from the planned External Audit Mode (which changes the audit *destination*, not alerting).
