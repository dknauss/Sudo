...
|----------|---------|------------|---------|
| `_wp_sudo_proofs` | Per-login-session proof map keyed by `sha256(verifier)`; each entry is `{ token, expires, hmac }`, self-authenticating via `hash_hmac('sha256', "$user_id\|$verifier\|$token\|$expires", wp_salt('auth'))` (5.0.0) | `Sudo_Session::activate()` | `Sudo_Session::is_active()`, `is_within_grace()` (read cache-bypassed) |
