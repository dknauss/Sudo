...
**"Code-capable" means effective capability** (`edit_plugins`/`edit_themes`/`upload_plugins`/`upload_themes`, which map to `install_*`), **not** role names — so custom roles and the multisite distinction fall out correctly.
...
| No actor **and** core's own automatic updater **and** package from the site's configured update source | **Allow** (background security updates keep working) |
...
1. The proof secret lives only in the browser that answered the challenge; stored server-side only as a hash, keyed to the **current login-session token verifier** (not per-user — concurrent sessions must not overwrite each other).
2. The assurance record is **self-authenticating**: `hash_hmac('sha256', "$user_id|$verifier|$reauth_at|$scope", wp_salt('auth'))`, checked on read — because `session_tokens` lives in the persistent, poisonable `user_meta` cache group, an unsigned stamp is forgeable by a cache-poisoning primitive (the wp2shell class; see [§9](#9-related-pre-authentication-chains-eg-wp2shell)). Documented degradation: weaker when the `AUTH_SALT` family lives in `wp_options` rather than `wp-config.php`.
...
Named and set aside — a *different* primitive or *not reachable through the core code effects above*: same-origin script-load via `siteurl`/`home` repointing; media-upload-to-PHP (`upload_filetypes`, multisite); the WXR importer; `wp-config.php` / drop-ins / direct DB writes; and hosting/FTP/SSH access (which strictly dominates the gate). **Not** bracketed: wordpress.org installs, updates, and activation — those are Group-A effects covered by the `install_package()`/`activate_plugin()` seams under the actor-class policy.
