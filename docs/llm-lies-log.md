...
Audited: 2026-07-29 (v4.9.2 pre-tag audit, #445/#463/#469/#436-face-2 — 4 new entries, #78-#81. All four are prose about code that was itself correct: not one of them would have changed a line of production behaviour, and not one was catchable by a test. #78 and #80 are unearned scope claims, and #80 was written while FIXING #78's sibling in the same PR — the fourth occurrence of correction-introduces-defect in this log (#49-#53, #73-#75, and both halves of this cut). #79 is the INVERSE of #47/#55/#59 rather than another instance of them — those were wrong enclosing symbols, a true claim wearing a false citation; here the citation is exact and the rationale beside it is invented. Both shapes live in the same blind spot: `verify:sources` checks that the cited snippet exists, never that the prose about it is true, and the prose is everything in a registry row except the anchor. #81 is mine, reported to the repo owner and corrected one step later. Caught by: spawned reviewer agents (3 — #78 in the ninth review round of the same change, #79 in the same pass, #80 in the third round of the next PR), and self-review (1 — #81). None by the test suite, PHPStan, Psalm, or metrics, none of which read prose. Two follow-ups filed from the audit: #506 for the route #78 falsely excluded, #508 for the integration coverage the unit tests do not provide.)
Previous: 2026-07-28 (v4.9.1 pre-tag audit, #478/#488/#492 replay-language sweep — 6 new entries, #72-#77. One claim (#72) survived in FOUR sites across three review rounds, each sibling found in a file the previous round had not read; its two "fixes" cited the wrong link in the chain and carried the absence claim forward, so the registry briefly asserted and disclaimed the same prediction one row apart. Three (#73-#75) are fabricated or spliced quotations in ONE changelog bullet, all from the same root cause: intermediate states of an unmerged branch described as though they were what v4.9.0 shipped — including #74, a "contradiction" that existed for hours on a branch and was inherited from my own commit message. #76 is a fully-cited release-blocking bug report about a worktree three merges stale. #77 is a fabricated mechanism handed over by one agent and propagated by a second into a filed issue as the basis for a scope decision. Caught by: spawned reviewer agents (5 — #72's two surviving siblings, each in a file the prior round had not read, plus #73-#75 in a single pass that read the v4.9.0 blobs instead of the branch), automated Codex review (2 — the registry-row citations in #72), a Phase 27 reviewer who ran the lane instead of reading it (#77), and self-review while cutting a worktree from the real tip (#76). None by the test suite, which cannot see prose. Note the shape of #73-#75: the errors were introduced while CORRECTING #72, which is the same pattern as #49-#53.)
...
72. AN ABSENCE CLAIM ABOUT AMBIENT GLOBAL STATE — "is_network_admin() is false there"
   ...
   Claim:  Shipped in v4.9.0: "This method runs under admin-ajax.php … which never
           calls set_current_screen() and does not define WP_NETWORK_ADMIN, so
           is_network_admin() returns false here even for a genuine network action —
           **a condition that cannot be true on the live path is not a guard.**"
           Restated in the registry as "which is false on the admin-ajax path where
           the landing is computed", and in a test docblock as "the live path runs
           under admin-ajax.php where it is false anyway".
   Reality: `is_network_admin()` reads `$GLOBALS['current_screen']`, then the
           `WP_NETWORK_ADMIN` constant, then falls through to `return false`. An
           unmodified `admin-ajax.php` sets neither — but both are ordinary global
           state that any earlier-running plugin or hook can set before WP Sudo's
           priority-10 handler runs. The value is **unpredictable**, not false.

           Worse for the original: the method has a SECOND live entry context. It is
           called only from `build_replay_response_data()`, which has exactly two
           callers — `replay_stash()` and `render_resume_page()`. The second arrives
           via the challenge page render, and `Challenge::register()` hangs that page
           off `network_admin_menu` when `is_multisite()`. So the condition the
           comment called impossible is reachable, and "the live path" in the definite
           singular denies a path that exists.
   Source: wordpress-develop trunk — `is_network_admin()` in `wp-includes/load.php`
           (three branches); `set_current_screen()` called at `wp-admin/admin.php:217`;
           `WP_Screen::get()` reading the constant to set `in_admin = 'network'`;
           `define( 'WP_NETWORK_ADMIN', true )` at `wp-admin/network/admin.php:10`.
           Read 2026-07-28.
   Notes:  Two correction attempts each made it worse in a new way. The first added
           two registry rows citing the CONSTANT branch — but on the network challenge
           page `admin.php` has already called `set_current_screen()`, so the function
           answers through the current_screen branch and the citations sourced a link
           in the chain that is not the one taken. The second row then carried the
           absence claim forward in its own note, so the registry simultaneously
           asserted and (one row later) disclaimed the same prediction.

           Closing it required going the other way: **deleting the claim rather than
           sourcing it.** The comment's argument never needed a value — "ambient to
           the request rather than a property of the URL" is sufficient to conclude
           the path must decide, and the mutability that made the prediction wrong is
           ...
           When the row was deleted the exemption was reverted, but a gate modified to
           admit a claim that is then withdrawn is a way for a retraction to leave the
           checker permanently looser than it was.
   Fix:    An absence claim about global state is unbounded unless the state is
           unreachable, and ambient state in WordPress is never unreachable — any
           earlier hook can set it. Say what the value IS read from, not what it will
           be. And when a citation is demanded for a claim, check first whether the
           claim is needed at all: four citations to support one sentence is a signal
           to delete the sentence.

73. A QUOTATION OF THE FIX PRESENTED AS THE DEFECT — the "before" text quoted in a
    changelog was the CORRECTED docblock, so anyone verifying the entry by grepping
    the quote would land on the fix and conclude it was never made.
   Files:  CHANGELOG.md (§ 4.9.1, "Documentation accuracy")
   Claim:  "`Challenge::clear_binding_cookie()` was documented as clearing **"on every
           path that consumes a stash, so a proof never outlives the stash"**."
   Reality: At v4.9.0 the docblock read "Cleared on **BOTH the replay and fail-closed
           paths** so a proof never outlives the stash it belonged to." The quoted
           phrase existed only in the corrected text written days later, on the branch
           the changelog was being drafted from.
   Source: `git show v4.9.0:includes/class-challenge.php`, read 2026-07-28.
   Notes:  The substance was right — the method does `if ( headers_sent() ) { return; }`
           before `setcookie()`, so the guarantee was false — but the evidence offered
           for it pointed at the wrong text. This is a self-defeating citation: the
           more diligently a reader checks it, the more certain they become that
           nothing was wrong.
   Fix:    Every "before" in a release note is a claim about a specific tree. Resolve
           it against the previous TAG's blob, never against the branch being edited.
           Prefer unquoted paraphrase — the sibling `readme.txt` entry described the
           same two defects without quoting and survived all three review passes.

74. A CONTRADICTION THAT NEVER EXISTED, INHERITED FROM MY OWN COMMIT MESSAGE — a
    narrative written on a branch, describing that branch's intermediate state, carried
    into a durable changelog as though it described the shipped release.
   Files:  CHANGELOG.md (§ 4.9.1, "Documentation accuracy")
   Claim:  "`Request_Stash::mint_binding_proof()` carried a live-tense rationale that
           **contradicted its own next paragraph declaring the binding inert.**"
   Reality: No such paragraph existed at v4.9.0. The word INERT does not appear in
           `class-request-stash.php` at that tag. What followed the stranding rationale
           was "Binding is defence-in-depth only. It is never sufficient on its own to
           authorize a replay — informed confirmation of the named target is the
           primary control" — itself stale live-tense prose about the retired design.
           Both paragraphs described the removed mechanism uniformly. There was no
           self-contradiction to find.

           The INERT paragraph was text an EARLIER COMMIT ON THE SAME BRANCH added.
           The "contradiction" was real for a few hours, on an unmerged branch, and was
           an artefact of a partial correction rather than a property of any release.
   Source: `git show v4.9.0:includes/class-request-stash.php`, read 2026-07-28.
   Notes:  The framing came from a review comment that was accurate when written —
           about the branch — was repeated in the commit message, and was then lifted
           into the changelog without re-checking what it was a claim ABOUT. Each
           restatement felt like quoting an established finding.

           A commit message describes a step on a branch. A changelog describes a
           release. They are claims about different trees, and prose moves between
           them without changing tense or scope to say so.
   Fix:    Treat your own prior prose as an unverified source. Inherited narrative is
           the easiest kind to propagate, because rechecking it feels like rechecking
           something already settled.

75. A QUOTED "BEFORE" STRING THAT NEVER EXISTED IN THE REPOSITORY — presented in both
    the changelog and the user-facing readme as the wording being corrected.
   Files:  CHANGELOG.md (§ 4.9.1), readme.txt (§ Changelog, `= 4.9.1 =`)
   Claim:  'Where a claim said **"nothing is replayed on any path"** it now says "no
           server-stashed request is replayed".'
   Reality: That string is not in the v4.9.0 tree in any casing. The real prior wording
           was the unqualified "nothing is replayed" / "never replayed". The quoted form
           was assembled from the shape of the correction rather than read from the text
           corrected.

           The sentence overstated in a second way: "is now" asserts a completed
           substitution. About fifteen unqualified instances legitimately remain, where
           the surrounding sentence already scopes the claim to the stash.
   Source: `git show v4.9.0:includes/class-challenge.php` and a repo-wide search of the
           tag, read 2026-07-28.
   Notes:  The reviewer's own falsifying evidence used `git log -S`, which is
           case-sensitive; a capitalised variant did exist on the branch. The conclusion
           was right, but the method would have missed it. Checking the tag's tree
           directly is what settled it — a reminder that a clean negative is only as
           good as the search that produced it (see #66, #69).
   Fix:    Quotation marks are a claim that a string exists somewhere specific. If the
           string was not copied from a file, do not put quotes around it. And do not
           write "X is now Y" without counting: say "several", or say which.

76. A FULLY-CITED BUG REPORT ABOUT A TREE NOBODY SHIPS — filed as an issue, called
    release-blocking, and entirely an artefact of reading a stale worktree.
   Files:  GitHub issue #489 (closed as invalid)
   Claim:  `readme.txt`'s 4.9.0 changelog still described the abandoned CONDITIONAL
           replay design — "requires the challenge to have named the whole effect",
           "generally not resumed automatically", "one case is not yet reliably
           refused", a cross-reference to a developer-reference section that no longer
           exists, and "`wp_sudo_action_replayed` is unchanged and still fires".
   Reality: Every quotation was real, and every one came from `/Users/dan-knauss/Code/Sudo`
           — the worktree named after the project — which sat three merges behind at
           `5e0cd4e`. On the actual tip the bullet already read "automatic replay …
           is removed outright" and named the hook as "retained but **dormant**". The
           upgrade notice already said "no longer auto-replayed **at all**". There was
           no defect and nothing release-blocking.
   Source: `git show origin/main:readme.txt` after `git fetch`, 2026-07-28.
   Notes:  The report was internally consistent and heavily evidenced: direct quotes,
           exact line numbers, and a cross-reference independently verified as dangling.
           Confidence scaled with citation quality, which is precisely what made it
           persuasive. Nothing in the evidence could reveal the tree was stale, because
           the staleness was in the premise rather than in any cited fact.

           The repo runs roughly thirty worktrees. The one named `Sudo` is not the
           current one; it is just another checkout, and long-lived checkouts drift
           furthest. A merge completed minutes earlier updates none of them.
   Fix:    Read facts through `git show origin/main:<path>` after a fetch, not from a
           working tree. For a sweep, `git grep <pattern> origin/main -- <paths>`. State
           which tree a claim is about whenever it could matter.

77. A DEFECT INVENTED TO EXPLAIN A KNOWN-BROKEN LANE — then propagated by a second
    agent into a filed issue as the justification for a deferral decision.
   Files:  .planning/phases/27-threat-and-trusted-flow/ (Phase 27 handoff prose),
           GitHub issue #490 (corrected in-thread)
   Claim:  The Phase 27 multisite lane fails because `DOMAIN_CURRENT_SITE` resolves
           against port 8893 while the proxy runs on 9444, so the lane "does not come
           up".
   Reality: Unsupported. `9444` appears in `bin/run-phase27-wordpress.mjs` inside a
           factual inventory that draws no such inference; the default was read without
           reading its sole caller. What the lane actually does is exit 1 on a missing
           config file. More decisively, the port could never have been the binding
           constraint: `PHASE27_MULTISITE` has exactly one definition
           (`run-phase27-wordpress.mjs:29`) and NOTHING sets it — no workflow, no npm
           script, no default — so the lane was unreachable through any committed entry
           point regardless of its contents. The workflow that would host it has never
           run anywhere: `git rev-list --all --count` for its path is 0.
   Source: `git grep PHASE27_MULTISITE -- ':!*.md'` and `git rev-list --all --count --
           .github/workflows/phase27-research.yml`, both 2026-07-28; the runner's own
           behaviour observed by execution.
   Notes:  Two failures compounded. The first agent inferred a mechanism from a port
           number. The second — me — wrote it into a GitHub issue as established fact
           without checking, and used it to argue a scope decision. The decision was
           right for other reasons, but it was argued from a fabrication, and the issue
           would have preserved that reasoning indefinitely.

           A reviewer found it by running the lane instead of reading it, and in doing
           so also found that the error surfacing to the user is a REPLACED exception:
           `stackStarted` is set before the throw, so the `finally` block's own failure
           overwrites the original error. The message a reader would attribute to the
           config is not the one the config produced.
   Fix:    A defect handed to you by another agent is a claim, not a finding — verify it
           before it enters an issue, a commit message, or a decision, because those are
           the places it stops being questioned. When explaining why something is broken,
           prefer the check that would prove it unreachable at all over the one that
           explains a symptom.

78. A SCOPE CLAIM THAT MADE A GUARD SOUND COMPLETE — asserted in three places at once,
    in the commit that added the guard.
   Files:  includes/class-action-registry.php (the `options.critical` admin.callback
           closure), CHANGELOG.md, tests/Unit/ActionRegistryTest.php (docblock of
           test_options_critical_callback_omission_guard_is_single_site_only)
   Claim:  A single-site General Settings save is "the only place default_role is
           allowlisted", so "on multisite an omitted default_role is never written".
   Reality: False on both halves. `wp-admin/options.php` also serves the
           unregistered-settings page: `$allowed_options['options'] = array();` is
           registered (trunk L159) so the allow-list gate passes, and inside the
           `'update' === $action` branch `$options` comes from a caller-supplied
           `page_options` field that never consults `$allowed_options` (trunk L263).
           So `option_page=options&page_options=default_role` with the field omitted
           reaches the same `update_option( 'default_role', null )`, on single site and
           multisite alike; multisite restricts the branch to `manage_network_options`
           but does not close it.
   Source: wordpress-develop trunk `src/wp-admin/options.php` and a local WP 7.0.1
           install, both read 2026-07-29. Now registered as GB-OPTIONS-PAGE-OPTIONS.
   Notes:  Found by a spawned reviewer agent in the NINTH review round of the same
           change. The guard itself was correct; only the sentence claiming its reach
           was wrong. Filed as #506 rather than widened, and the prose narrowed to
           bound the SCREEN rather than the option.
   Fix:    A guard's scope sentence is a claim about every OTHER route to the same
           effect, not about the route in front of you. Enumerate the callers of the
           sink before writing "the only".

79. TWO REGISTRY ROWS WHOSE ANCHOR WAS RIGHT AND WHOSE REASON WAS INVENTED.
   Files:  docs/upstream-sources.md (GB-SANITIZE-OPTION-RESTORE, GB-DEFAULT-ROLE-NULL)
   Claim:  (a) The restore statement "appears twice in the function — here and in the
           `admin_email` case". (b) `subscriber` "also appears ... in unrelated
           branches, so the assignment alone would be an ambiguous citation".
   Reality: (a) The second `$value = get_option( $option );` is at trunk L5081 inside
           `case 'WPLANG':`. The `admin_email`/`new_admin_email` case makes no
           `get_option()` call at all — it sets `$error` and depends on the very block
           the row documents, which is what the row exists to say. (b) `subscriber`
           occurs exactly twice, L5156-5157, both inside `case 'default_role':`; either
           line would have been unique, so there was no ambiguity to disambiguate.
   Source: wordpress-develop trunk `src/wp-includes/formatting.php`, read 2026-07-29;
           `grep -o subscriber | wc -l` returns 2.
   Notes:  Both anchors were correct and `verify:sources` passed throughout — it checks
           that the cited snippet exists, not that the sentence about it is true. Note
           this is the INVERSE of #47/#55/#59, not another of them: those were wrong
           enclosing symbols, where the claim was true and the citation false. Here the
           citation is exact and the prose beside it is invented. What the two shapes
           share is the column that gets no machine check — which is the whole of the
           registry except the anchor.
   Fix:    The registry's "why this anchor" field is prose and gets no machine check.
           Read the other occurrences before claiming what they are.

80. A UNIQUENESS CLAIM WRITTEN WHILE FIXING A UNIQUENESS CLAIM — and falsified by the
    rule named two sentences above it.
   Files:  bridges/wp-sudo-two-factor-lifecycle-bridge.php (the
           $profile_provider_changed rule's stash comment)
   Claim:  The bridge rule is "the one place the flag can be raised WITH a credential
           present — no builtin rule can, since every credential-carrying one uses
           stash_no_replay()".
   Reality: `user.create` (includes/class-action-registry.php:463) is a builtin using
   ...
   Notes:  Introduced in the fix for a review finding about a DIFFERENT false claim in
           the same comment. Second time in the same PR that a correction shipped a new
           defect, and the same pattern as #49-#53 and #73-#75. The replacement states
           the one-way mechanism instead of a uniqueness boast, and keeps only the
           narrow claim an enumeration supports.
   Fix:    When rewriting a sentence flagged as an unearned scope claim, the danger is
           writing a second one that feels earned because you just thought about it.
           Prefer stating the mechanism; mechanisms do not need a scope word.

81. A RELEASE STEP DECLARED UNDONE BECAUSE A BRANCH FOR IT STILL EXISTED.
   Files:  session report to the repo owner (not committed)
   Claim:  The v4.9.1 pre-tag external-claim audit "was never PR'd or landed" — asserted
           on finding an unmerged branch `docs/lies-log-4.9.1-audit` holding it.
   Reality: It had landed. `main`'s `docs/llm-lies-log.md` already carried the
           "Audited: 2026-07-28 (v4.9.1 pre-tag audit ... 6 new entries, #72-#77)"
           header and entries #72-#77; main was 2,196 lines to the branch's 2,184. The
           branch is a stale duplicate left behind after the content merged by another
           route.
   Source: `grep` for the audit header in main's copy, and `wc -l` on both, 2026-07-29.
   Notes:  Caught by self-review one step later, while resolving the cherry-pick
           conflict the duplicate caused — the conflict was the evidence. Reported as a
           correction in the same session. An unmerged branch is evidence about a
           branch, not about main; on a repo where several sessions work at once and
           leftover branches are normal, it is close to no evidence at all.
   Fix:    Check the destination, not the vehicle. Before reporting work as missing,
           grep for its OUTPUT on the target branch.
