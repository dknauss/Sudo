...
The central idea is this: **you do not have to guard every screen, route, and REST endpoint that can change a password or install a plugin — you guard the one core function they all call.** WordPress funnels each consequential effect through a small number of data-layer functions (`wp_update_user()`, `activate_plugin()`, `WP_Upgrader::install_package()`, and a few more). Put the check *inside* that function, before the write, and the admin form, the REST call, the bulk action, and a plugin calling the function directly are **all** covered by the same few lines, because they all pass through the same place.

The check itself is small. Most of these functions already return a `WP_Error` on failure, and every caller already knows how to handle that. So the guard is: *is a fresh, deliberate re-authentication on record for this session? If not, return a `WP_Error` that means "reauthenticate first."* The browser turns that error into a challenge page and, on success, replays the original request; REST turns the same error into a 403 with a link to the challenge. No new plumbing is needed for the callers: the challenge rides the error path that is already there. (A few chokepoints do not return `WP_Error`: `wp_delete_user()` returns `bool` and `switch_theme()` returns `void`. Those get a small pre-effect adapter instead; the spec §5.1 spells out which is which.)

The four sketches below show that end to end: (1) how the "recent-auth on record" is stored so a stolen cookie can't forge it, (2) the helper that answers the yes/no question, (3) the one-guard-per-chokepoint insertions, and (4) the browser adapter that renders the challenge and replays. **These are illustrative sketches to make the shape concrete — they have not been run against a real core checkout, and the line anchors are approximate.** Read them as "this is roughly what the patch looks like," not as a tested patch.
