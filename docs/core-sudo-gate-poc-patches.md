...
If you only take one idea from this document, take this: **you do not have to guard every screen, route, and REST endpoint that can change a password or install a plugin — you guard the one core function they all call.** WordPress funnels each consequential effect through a small number of data-layer functions (`wp_update_user()`, `activate_plugin()`, `WP_Upgrader::install_package()`, and a few more). Put the check *inside* that function, before the write, and the admin form, the REST call, the bulk action, and a plugin calling the function directly are **all** covered by the same few lines — because they all pass through the same place.

The check itself is small. Most of these functions already return a `WP_Error` on failure, and every caller already knows how to handle that. So the guard is: *is a fresh, deliberate re-authentication on record for this session? If not, return a `WP_Error` that means "reauthenticate first."* The browser turns that error into a challenge page and, on success, replays the original request; REST turns the same error into a 403 with a link to the challenge. No new plumbing for the callers — the challenge rides the error path that is already there. (A few chokepoints don't return `WP_Error` — `wp_delete_user()` returns `bool`, `switch_theme()` returns `void` — so those get a small pre-effect adapter instead; the spec §5.1 spells out which is which.)
