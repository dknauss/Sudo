...
When a user attempts a gated action — for example, activating a plugin — Sudo intercepts the request before WordPress processes the sensitive operation. In many common browser-admin flows this happens on `admin_init`; other actions use surface-specific hooks. The original request is stashed in a transient and the user is redirected to a challenge page that names the action and its concrete target (for example the specific plugin or account). After successful reauthentication the request is resumed **only** when the browser presenting the reauthentication is the one that created the stash — proven by a `__Host-` cookie set when the action started, which browsers accept only over HTTPS. Otherwise, and on any non-HTTPS site, nothing is resumed: the user is returned to where they were with a "review and submit again" notice and performs the action themselves, now covered by the active sudo session. This is what closes the confused-deputy risk — a stolen session can plant an action, but only the browser that planted it can ever have it resumed, and the challenge names what is being authorized. Passwords, tokens, API keys, and other configured secret fields are omitted from the stash, and such requests are never resumed automatically. For AJAX and REST requests, the browser receives a `sudo_required` error, and an admin notice appears on the next page load linking to the challenge page. The user authenticates, activates a sudo session, and retries the action.
