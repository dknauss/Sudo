...
**The primary threat WP Sudo is built for:** an attacker has a valid authenticated admin session — stolen cookie, unattended browser, hijacked session via XSS — but does not know the account password or second factor, and no active sudo window is already open. On that threat, WP Sudo is highly effective: the attacker cannot *silently* perform a covered high-risk operation — each requires completing the reauthentication challenge (the account password or second factor), which a session-only thief lacks. One honest caveat remains: an XSS that is *active at the reauthentication moment* can ride the window or keylog the challenge. The related confused-deputy issue — where an attacker planted an action and the victim's reauthentication carried it out — is closed: a stashed action is never resumed automatically unless the browser presenting the reauthentication is the one that created it, and the challenge now names the concrete target so the credential is consent to a known action rather than a blank cheque. Sudo is strong against a remote stolen or cloned cookie; it is not a defense against live same-origin script at reauth time.
...
When a user attempts a gated action — for example, activating a plugin — Sudo intercepts the request before WordPress processes the sensitive operation. In many common browser-admin flows this happens on `admin_init`; other actions use surface-specific hooks. The original request is stashed in a transient and the user is redirected to a challenge page that names the action and, where the request carries a parameter WP Sudo recognises as a target, that concrete target (for example the specific plugin or account). After successful reauthentication the request is resumed **only** when two things hold: the challenge was able to name the whole effect, and the browser presenting the reauthentication is the one that created the stash — proven by a `__Host-` cookie set when the action started. That cookie is only issued on a site WP Sudo considers secure (`is_ssl()` or `force_ssl_admin()`) — see [Security Model → HTTPS and reauthentication replay](security-model.md#environmental-considerations) for why the check is WP Sudo's rather than the browser's. Otherwise, and on any non-HTTPS site, nothing is resumed: the user is returned to where they were with a "review and submit again" notice and performs the action themselves, now covered by the active sudo session. This is what closes the confused-deputy risk — a stolen session can plant an action, but only the browser that planted it can ever have it resumed, and the challenge names what is being authorized.

**When the challenge cannot name the whole effect, nothing is resumed.** An action whose effect is carried in a parameter WP Sudo does not recognise as a target renders no target line, so consenting to it would be consenting to a category rather than an action. Such requests always take the manual path instead: full-site export is the built-in example, and custom rules whose effect field is not a recognised target behave the same way. This is the canonical description of when a request is and is not resumed; `readme.md`, `readme.txt` and `docs/two-factor-integration.md` describe the same flow in less detail and defer here. The parameter list and the rule-author contract are in [`docs/developer-reference.md`](developer-reference.md); a known gap in that guarantee for GET requests is tracked in issue #412. Passwords, tokens, API keys, and other configured secret fields are omitted from the stash, and such requests are never resumed automatically. For AJAX and REST requests, the browser receives a `sudo_required` error, and an admin notice appears on the next page load linking to the challenge page. The user authenticates, activates a sudo session, and retries the action.
