...
**The primary threat WP Sudo is built for:** an attacker has a valid authenticated admin session — stolen cookie, unattended browser, hijacked session via XSS — but does not know the account password or second factor, and no active sudo window is already open. On that threat, WP Sudo is highly effective: the attacker cannot *silently* perform a covered high-risk operation — each requires completing the reauthentication challenge (the account password or second factor), which a session-only thief lacks. Two honest caveats: completing a challenge proves a *fresh factor was supplied*, not necessarily intent for one specific transaction — an attacker who lures the legitimate user to a crafted challenge URL and reauthenticates within the short stash window can have a *pre-planted* action replayed (a confused-deputy issue under active hardening); and an XSS that is *active at the reauthentication moment* can ride the window or keylog the challenge. Sudo is strong against a remote stolen or cloned cookie; it is not a defense against live same-origin script at reauth time.
