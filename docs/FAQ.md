...
> **Research prototype:** WP Sudo is not a production security plugin. Use it
> only in WordPress Playground or disposable local tests with synthetic data.
> See [Project Status](../PROJECT-STATUS.md).

## What problem does Sudo solve?

**The primary threat WP Sudo is built for:** an attacker has a valid authenticated admin session — stolen cookie, unattended browser, hijacked session via XSS — but does not know the account password or second factor, and no active sudo window is already open. On that threat, WP Sudo is highly effective: the attacker cannot *silently* perform a covered high-risk operation — each requires completing the reauthentication challenge (the account password or second factor), which a session-only thief lacks. One honest caveat remains: an XSS that is *active at the reauthentication moment* can ride the window or keylog the challenge. The related confused-deputy issue — where an attacker planted an action and the victim's reauthentication carried it out — is closed: automatic replay was **removed outright rather than made conditional**, so no branch of the challenge can return a replay instruction for a server-stashed request. After reauthenticating you re-issue the action yourself, and the challenge names the concrete target so the credential is consent to a known action rather than a blank cheque. Sudo is strong against a remotely stolen WordPress authentication cookie; it is not a defense against live same-origin script at reauth time, nor against a copy of the browser's complete cookie state, which carries the `wp_sudo_token` binding cookie too.
...
comparison with other reauthentication approaches, see the
[Architecture Comparison Matrix](sudo-architecture-comparison-matrix.md).

## Does Sudo complement or compete with WordPress security plugins?

Conceptually, recent-authentication gates could complement authorization,
strong authentication, vulnerability remediation, and perimeter controls.
This repository can demonstrate those interactions in an isolated test
environment, including integration with the Two Factor plugin. It must not be
deployed as an additional defense on a real site.
...
When a user attempts a gated action — for example, activating a plugin — Sudo intercepts the request before WordPress processes the sensitive operation. In many common browser-admin flows this happens on `admin_init`; other actions use surface-specific hooks. The original request is stashed in a transient and the user is redirected to a challenge page that names the action and, where the request carries a parameter WP Sudo recognises as a target, that concrete target (for example the specific plugin or account). After successful reauthentication **no server-stashed request is resumed**: the stash is discarded and the user is returned to where they were (handler URLs such as `options.php` land on the dashboard instead, and `user-edit.php` on the Users list — a bare GET of them renders nothing usable) with a "review and submit again" notice, to perform the action themselves under the sudo session they just earned. `Challenge::build_replay_response_data()` has no branch that can return a replay instruction, which is the whole guarantee and is verifiable by reading one method. This is what closes the confused-deputy risk — an attacker who plants a stashed action gains nothing from the victim's reauthentication, because the plugin never carries out a stashed request at all. An interim design authorised replay per-request behind a `__Host-` binding cookie plus a confirmation naming the target; 4.9.0 **removed automatic replay rather than conditioning it** (#322).

**No server-stashed request is ever resumed automatically — the user always re-issues the action.** This holds for every method, every rule and every admin surface that stashes; there is no eligibility test to satisfy and no remaining category of request that replays. The challenge still names the concrete target wherever the request carries a parameter WP Sudo recognises as one, but that target line exists for *informed consent* — so the credential authorises a known action rather than a category — not to qualify anything for replay. This is the canonical description of when a request is and is not resumed; `readme.md`, `readme.txt` and `docs/two-factor-integration.md` describe the same flow in less detail and defer here. The recognised-parameter list and the rule-author contract are in [`docs/developer-reference.md`](developer-reference.md). Passwords, tokens, API keys, and other configured secret fields are omitted from the stash. For AJAX and REST requests, the browser receives a `sudo_required` error, and an admin notice appears on the next page load linking to the challenge page. The user authenticates, activates a sudo session, and retries the action.

One deliberate non-exception: the block editor's in-tab retry. Nothing is stored server-side there — the request was dispatched from the tab, came back `sudo_required`, and that rejection is what opens the modal — and the re-dispatch runs only for the caller that opened it, holding its own request options in a closure. Nobody else can plant that request, which is precisely what made a *server* stash a confused deputy.
...
change live gate state. There is no resumption for it to predict either: since
#322 no server-stashed request is resumed after reauthentication, so the outcome
is always that the user re-issues the action themselves.
...
When sudo is activated, a cryptographic token is stored in a secure httponly cookie, and its hash is saved in user meta. On every gated request, both must match. A stolen WordPress **authentication** cookie on a different browser will not have a valid sudo session — the attacker lacks the second cookie, `wp_sudo_token`. A copy of the browser's complete cookie state carries both and is **not** covered; see [Security Model](security-model.md#boundary-session-binding-vs-a-cloned-cookie-jar).
...
One caveat worth knowing: on shared terminals, a password login that is *not* completed with a session-rotating second factor leaves a sudo window open for whoever sits down next. (A Two Factor-enrolled login does not — WP Sudo binds the grant to the login session, and Two Factor destroys that session to challenge the factor, which invalidates the grant, so the enrolled user is re-challenged rather than left with a password-strength window.) Sites that care can suppress the automatic grant with the `wp_sudo_grant_session_on_login` filter (since 3.3.0) — return `false` and users must pass an explicit challenge at their first gated action instead. See the [security model](security-model.md#login-auto-grant) for the full analysis.
...
**What it does not relax:** session binding. A stolen WordPress authentication cookie on a different browser does not gain grace-period access (a copy of complete cookie state would, since it carries `wp_sudo_token` too — the same boundary as the sudo window itself). The self-authenticating proof (HMAC + cookie match) must still verify — `is_within_grace()` resolves and validates it before returning true. The admin bar timer always reflects the true session state (`is_active()`), not the grace state — the user sees accurately when their session has expired.
