...
- The **credential-manufacturing pivots** are gated: account-email change is now closed by the `user.change_email` rule (§4), and the REST rules for email/password/role gate **`POST` as well as `PUT`/`PATCH`** (core's `WP_REST_Server::EDITABLE = 'POST, PUT, PATCH'`) — a gap a prior version of these rules missed, so a stolen cookie could `POST /wp/v2/users/{id}` ungated.
