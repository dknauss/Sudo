...
- A rule whose effect is carried in a parameter WP Sudo does not recognise as a target
  ...
  (`checked`), the critical option keys, and app-password names. Note the
  critical-option set and the recognised-target set are **independent**: the former is
  filterable via `wp_sudo_critical_options`, so adding an option there does not make it
  a recognised target, and a target key need not be a gated option. (`new_admin_email`
  was the standing example of the gap until 4.9.0, when it was added to
  `TARGET_PARAMS` — `options-general.php` posts the administration email under that
...
// Fires when an operator clears reauth failure state via the WP-CLI
...
//
// Fires whenever the command actually discarded something, which is BROADER
// than "was locked out": a user with sub-threshold failures, an active
// throttle, or an expired-but-uncleared lockout also loses that state, and
// that is equally worth auditing. $was_locked is the discriminator — true
// only when a hard lockout was active at invocation. Do NOT read the hook
// alone as "this user was locked out".
//
// Fires even when NO per-user counter is tracked: the command still bumps
// the per-IP failure generation unconditionally in that case (the only lever
// that reaches windows raised on another blog and orphaned there by an
