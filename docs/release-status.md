...
- **Runtime version constant:** `4.9.2` on `main`. `WP_SUDO_VERSION` is set in `wp-sudo.php` (header + constant), `tests/bootstrap.php`, and `phpstan-bootstrap.php`; `readme.txt` Stable tag is `4.9.2`. All five version-sync points are in sync at `4.9.2`. **`blueprint.json` targets the published `v4.9.2` `wp-sudo.zip` release asset** (advanced after the GitHub Release was published and the asset verified): per [`VERSIONING.md`](../VERSIONING.md) the Playground install target is bumped **after** the release asset exists, never merely after the tag is cut, because the public "Try latest release" badge loads `blueprint.json` from `main` and an earlier bump would point the demo at a ZIP that does not exist yet. The lag is the policy working, not a drift.
