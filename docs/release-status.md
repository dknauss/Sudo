...
- **Unreleased work on `main` past `v4.8.0`:** one small feature plus documentation and CI tooling — none version-bumped. **Feature:** the optional critical-event alert bridge now pushes a high-severity alert on role/capability drift (`wp_sudo_role_drift_detected` → bridge push), a backward-compatible addition in the optional companion bridge (PR #226). **Docs/CI:** the 4.8.0 live/manual security-test checklist + results (#223–#225); core-proposal, roadmap, and accuracy docs (#227–#234); the **ROADMAP restructure** to a forward-only Now/Next/Later with the design essays promoted to standalone docs and the feature backlog migrated to GitHub issues **#238–#257** (PR #236; the Pass-3 roadmap trim #258 is a still-open PR, not yet on `main`); and the **persistent-options metrics gate** — a tokenizer-based dev/CI scanner (`bin/scan-persistent-options.php`) wired into `composer verify:metrics` (PR #237). None bump `WP_SUDO_VERSION`; the next tag's patch-vs-minor call is a maintainer decision. **This bullet is a convenience summary — the canonical, always-current drift source is `git log v4.8.0..main --oneline`.**
