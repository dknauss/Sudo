...
- **Latest tagged research preview:** `4.9.1` (cut 2026-07-28).
- **Latest git tag observed:** `v4.9.1` (annotated, cut 2026-07-28, on `336d095`, the squash-merge of PR #496). Its GitHub release and install asset are classified as a prerelease/research snapshot for disposable evaluation only.
...
- **Previous tag:** `v4.9.0` (annotated, cut 2026-07-27, on `82d4b23`, the squash-merge of PR #459).
...
- **Current `main` version:** `4.9.2` (runtime constant) — **not yet released.** `main` now **leads** the latest tag (`v4.9.1`, `336d095`) on the version constant: this is the pre-tag half of cutting `4.9.2`, so the version decision for the accumulated work HAS been made and the tag does not exist yet. `blueprint.json` still targets `tags/v4.9.1.zip` by policy until it does. The payload being cut: `wp_sudo_require()` reports its inert `return_url` argument through `_deprecated_argument()` (#461), the handler/usable-screen guard now asserts its two classifications are disjoint (#434), `options.critical` gates on a change to a critical option rather than on the field's presence (#445) — a behaviour change that also stops firing `wp_sudo_action_gated` for a General Settings save touching nothing critical — and the post-reauthentication landing notice is replaced by one string true on every path, carrying `role="alert"` (#463, #469, #436 face 2). The canonical drift source is `git log v4.9.1..main --oneline`, not this summary.
- **Prior `main` version:** `4.9.0` (runtime constant) — released on 2026-07-27 as `v4.9.0` (`82d4b23`), superseded by `4.9.1`. Superseding the prior state, in which the latest tag was `v4.8.0` (cut 2026-07-23, on `10587a4`). `main` was bumped off the released identity on 2026-07-26 so that an unreleased tree stops advertising a shipped version; the bump carries no payload of its own. At the time it was current, `main` had progressed **past** the tag with unreleased work; that work shipped in `v4.9.0` and the bullet recording it is marked HISTORICAL below. (`4.8.0` is the REST-gate security-hardening release plus the opt-in role/capability lockdown audit MVP; see the payload note above.)
...
- **`readme.txt` stable tag:** package/release metadata for generated plugin zips **only**. With submission not planned it carries **no WordPress.org contract** — it selects no served SVN tag, so it may legitimately lead the latest git tag while `main` is unreleased — and currently does: Stable tag `4.9.2` on `main`, latest tag `v4.9.1`, because the version sync for `4.9.2` has landed and the tag has not been cut yet. It does not indicate that this plugin is live in the WordPress.org repository.
...
Canonical source for post-tag drift after the latest tag (`v4.9.1`): `git log v4.9.1..main --oneline` (**now a real payload, not housekeeping**: #500, #504, #503, #484, #516, plus the `v4.9.1` post-tag commit and this `4.9.2` version sync. That payload is what `4.9.2` cuts — see the "Current `main` version" bullet above. The note that follows described the `v4.8.0` era and is retained as history: it was **not empty** — `main` carries the unreleased alert-bridge role-drift push (#226), scoped break-glass recovery (#240), the in-editor session-status indicator (#262), plus docs; see "HISTORICAL — what was unreleased on `main` past `v4.8.0`" above).
...
`v4.9.0` (2026-07-27) and `v4.9.1` (2026-07-28) (the canonical current state is "Latest
