...
- **Current `main` version:** `4.9.1` (runtime constant) — **released** on 2026-07-28 as `v4.9.1` (`336d095`, the squash-merge of PR #496). `main` and the latest tag agree on the version constant. The only commit after the tag is this post-tag housekeeping commit, which carries no payload. The canonical drift source is `git log v4.9.1..main --oneline`, not this summary.
