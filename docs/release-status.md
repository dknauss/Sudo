...
- `wp_sudo_action_replayed` keeps its signature `(int $user_id, string $rule_id)`. With
  automatic replay removed there is nothing left for it to announce, so it never fires:
